...
def main():
    ...
    if problem == 1:
        ...
        def evalg(x):
            ...
            return g
        def evalh(x):
            H = array([[2*(-5.1*x[0] / (2*math.pi**2) + 5/math.pi)**2 - 5.1*(x[1] - 5.1*x[0]**2 / (4*math.pi**2) + 5*x[0] / math.pi -6) / math.pi**2 - 10*(1 - 1/ (8*math.pi))*math.cos(x[0]),2*(-5.1*x[0] / (2*math.pi**2) + 5/math.pi)],[2*(-5.1*x[0] / (2*math.pi**2) + 5/math.pi),2]])
            return H
        ...
        while True:
            ...
            if method == 1: #最急降下法
                ...
                if problem == 1 or problem == 2:
                    print 'Error!'
                    return 0
                if problem == 4: #ニュートン法と同様に降下方向を決める
                  tau = 0
                  B = evalh(x)
                  I = identity(n)
                  while True:
                    try: #Lをコレスキー分解し降下方向を計算
                        ...
                        if tau == 0:
                          ...
                          tau = tau * 2
                elif problem == 5: #最急降下法と同様に降下方向を決める
                  d_prime = -g

                s = 1
                d = fmax(l,fmin(x + s * d_prime,u)) - x #射影勾配法により降下方向を決める
